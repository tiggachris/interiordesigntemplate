    ...
    <div className="pt-32 pb-24 min-h-screen bg-white">
      <div className="container mx-auto px-6 max-w-6xl">
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="text-center mb-24">
          <motion.p variants={fadeUp} className="text-softGold uppercase tracking-[0.2em] mb-4 text-sm font-medium">Our Story</motion.p>
          <motion.h1 variants={fadeUp} className="font-serif text-5xl md:text-6xl text-matteBlack mb-8">Crafting Timeless Elegance</motion.h1>
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="grid md:grid-cols-2 gap-16 mb-24 items-center">
          ...
          <div>
            <motion.h3 variants={fadeUp} className="font-serif text-3xl mb-6">Our Design Philosophy</motion.h3>
            ...
            <motion.div variants={fadeUp} className="grid grid-cols-2 gap-8 mt-12 border-t border-gray-200 pt-8">
              <div>
                <h4 className="font-serif text-4xl text-softGold mb-2">15+</h4>
                <p className="text-xs uppercase tracking-widest text-gray-400">Years Experience</p>
              </div>
              <div>
                <h4 className="font-serif text-4xl text-softGold mb-2">200+</h4>
        ...
        <div className="text-center mb-16">
          <h2 className="font-serif text-4xl mb-12">The Aura Process</h2>
          <div className="grid md:grid-cols-4 gap-8">
            {['Consultation', 'Concept & 3D', 'Execution', 'Handover'].map((step, idx) => (
              <motion.div key={idx} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="p-8 border border-gray-100 hover:shadow-2xl transition-shadow bg-beige/10">
                <span className="text-softGold font-serif text-3xl mb-4 block">0{idx + 1}</span>
                <h4 className="font-bold uppercase tracking-wider text-sm mb-2">{step}</h4>
              </motion.div>
            ))}
          </div>
        </div>
